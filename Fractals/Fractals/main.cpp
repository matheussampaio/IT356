...
int main(int argc, char *argv[])
{
    ...
    sf::RenderWindow window(sf::VideoMode(800, 800), "Hello World", sf::Style::Default, contextSettings);
...
void processEvent(sf::Event event,sf::RenderWindow& window)
{
	...
	case sf::Event::MouseMoved:
		if (mousePressed)
		{
			v.translate(sf::Mouse::getPosition(window).x - mouseX,window.getSize().y - sf::Mouse::getPosition(window).y - mouseY);
			mouseX = sf::Mouse::getPosition(window).x;
			mouseY = window.getSize().y - sf::Mouse::getPosition(window).y;
		}
		break;
