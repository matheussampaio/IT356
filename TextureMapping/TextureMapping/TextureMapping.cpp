...
int main(int argc, char *argv[])
{
    ...
    sf::RenderWindow window(sf::VideoMode(800, 600), "Texture Mapping", sf::Style::Default, contextSettings);
	resize(800,600);
  //  window.setVerticalSyncEnabled(true);

    // Make it the active window for OpenGL calls
 	window.setActive();   
    window.setFramerateLimit(120.0);
...
void processEvent(sf::Event event,sf::RenderWindow& window)
{
	...
	case sf::Event::MouseMoved:
		if (mousePressed)
		{
			v.mousemove(sf::Mouse::getPosition(window).x,window.getSize().y - sf::Mouse::getPosition(window).y);
			mouseX = sf::Mouse::getPosition(window).x;
			mouseY = window.getSize().y - sf::Mouse::getPosition(window).y;
		}
		break;
